...
export const removeHitFromFeed = async (objectID: string) => {
  ...
  try {
    ...
    if (hit) {
      ...
      const result = await db.runAsync(`DELETE FROM hits WHERE objectID = ?`, objectID);

      if (result.changes > 0) {
        console.log(`Item with objectID: ${objectID} removed from hits and added to deleted.`);
      } else {
    ...
    console.error("Error removing item from hits and adding it to deleted:", error);
  }
};
